...
describe('arrayUtils', function(){
  ...
  describe('growArray', function(){
    it('should not change size if new size <= current size', function(){
      const A: number[] = [0,1,2,3,4];
      util.growArray(A, 2, 4);
      assert.equal(A.length, 5);
      util.growArray(A, 2, 5);
      assert.equal(A.length, 5);
      util.growArray(A, 5, 5);
      assert.equal(A.length, 5);
    });

    it('should change to max size if valid and > current size', function(){
      const A: number[] = [0,1,2,3,4];
      util.growArray(A, 2, 10);
      assert.equal(A.length, 10);
    });

    it('should change to min size if > current size and max size invalid', function(){
      const A: number[] = [0,1,2,3,4];
      util.growArray(A, 10, Globals.MAX_PRECISION + 1);
      assert.equal(A.length, 10);
    });

    it('should throw error if new sizes > supported sizes', function(){
      const A: number[] = [0,1,2,3,4];
      const max: number = Globals.MAX_PRECISION;
      expect(function(){util.growArray(A, max + 1, max + 1)}).to.throw(RangeError);
    });
  });
