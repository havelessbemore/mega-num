...
describe('KaratsubaSquareMethod', function(){
  it('should return 1^2 in base 10', function(){
    const A: number[] = [1];
    const len: number = A.length = KaratsubaSquareMethod(A, A.length, 10);
    assert.equal(len, 1);
  ...
  it('should return 1234567^4 in base 10', function(){
    ...
    let len: number = actual.length = KaratsubaSquareMethod(actual, actual.length, 10);
  ...
  it('should return 9^2 in base 11', function(){
    ...
    let len: number = actual.length = KaratsubaSquareMethod(actual, actual.length, 11);
    assert.equal(len, expected.length);
    assert.deepEqual(actual, expected);
  });
  ...
  it('should return 9999999^2 in base 10000000', function(){
    ...
    let len: number = actual.length = KaratsubaSquareMethod(actual, actual.length, 10000000);
    assert.equal(len, expected.length);
    assert.deepEqual(actual, expected);
  });
});
