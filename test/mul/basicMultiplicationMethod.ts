...
describe('BasicMultiplicationMethod', function(){
  ...
  it('should return 1*1 in base 10', function(){
    ...
    const B: number[] = [1];
    const C: number[] = [1];
    const len: number = A.length = BasicMultiplicationMethod(A, A.length, B, B.length, 10);
  ...
  it('should return 456789 * 123 in base 2', function(){
    ...
    const len: number = A.length = BasicMultiplicationMethod(A, A.length, B, B.length, 2);
    assert.equal(len, C.length);
    assert.deepEqual(A, C);
  });
  ...
  it('should return 987654 * 321 in base 179', function(){
    ...
    const len: number = A.length = BasicMultiplicationMethod(A, A.length, B, B.length, 179);
    assert.equal(len, C.length);
    assert.deepEqual(A, C);
  });
